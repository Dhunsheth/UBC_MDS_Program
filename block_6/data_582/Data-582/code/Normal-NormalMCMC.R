...
for (i in 1:nIter){
  ...
  likelihood_current = dnorm(theta.current, mean = dat$mean, sd = sig/sqrt(n))
  likelihood_proposal = dnorm(theta.new, mean = dat$mean, sd = sig/sqrt(n))
  
  # Compute prior probability of current and proposed theta        
  prior_current = dnorm(theta.current, hparams$m, hparams$s)
  prior_proposal = dnorm(theta.new, hparams$m, hparams$s)
...
# save(chain, file = "NormalNormalChain.RData")
